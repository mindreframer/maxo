defmodule Mix.Tasks.Maxo.Gen.Live do
  ...
  types.  See `mix help maxo.gen.schema` for more information on attributes.
  ...
  Alternatively, the `--context-app` option may be supplied to the generator:
      ...
      mix maxo.gen.live Accounts User users --context-app warehouse
  ...
  module name, for example:
      ...
      mix maxo.gen.live Accounts User users --web Sales
  ...
  use binary ids for primary keys, see `mix help maxo.gen.schema` for more
  information.
  """
  use Mix.Task
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise(
        "mix maxo.gen.live must be invoked from within your *_web application root directory"
      )
    end
  ...
  defp files_to_be_generated(ctx = %Context{schema: schema, context_app: _context_app}) do
    ...
    web_live = Path.join([ctx.dir, "live", web_path, live_subdir])
    # test_live = Path.join([test_prefix, "live", web_path])
    test_live = web_live

    [
      {:eex, "show.ex", Path.join(web_live, "show.ex")},
      {:eex, "show_html.ex", Path.join(web_live, "show_html.ex")},
      {:eex, "index.ex", Path.join(web_live, "index.ex")},
      {:eex, "index_html.ex", Path.join(web_live, "index_html.ex")},
  ...
  defp copy_new_files(%Context{} = context, binding, paths) do
    ...
    Mix.Maxo.copy_from(paths, "priv/templates/maxo.gen.live", binding, files)
  ...
  def print_shell_instructions(%Context{schema: schema, context_app: ctx_app} = context) do
    ...
    web_path = Mix.Maxo.web_path(ctx_app)
  ...
  defp label(key), do: Maxo.Naming.humanize(to_string(key))
end
