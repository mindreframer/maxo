defmodule Mix.Tasks.Maxo.Gen.Context do
  ...
  Generates a context with functions around an Ecto schema.
      ...
      $ mix maxo.gen.context Accounts User users name:string age:integer
  ...
  a `--table` option may be provided. For example:
      ...
      $ mix maxo.gen.context Accounts User users --table cms_users
  ...
  Read the documentation for `maxo.gen.schema` for more information on
  attributes.
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise(
        "mix maxo.gen.context must be invoked from within your *_web application root directory"
      )
    end

    {context, schema} = build(args)
    binding = [context: context, schema: schema]
    paths = Mix.Maxo.generator_paths()
  ...
  def ensure_context_file_exists(%Context{file: file} = context, paths, binding) do
    unless Context.pre_existing?(context) do
      Mix.Generator.create_file(
        file,
        Mix.Maxo.eval_from(paths, "priv/templates/maxo.gen.context/context.ex", binding)
      )
    end
  end
  ...
  defp inject_schema_access(%Context{file: file} = context, paths, binding) do
    ...
    paths
    |> Mix.Maxo.eval_from(
      "priv/templates/maxo.gen.context/#{schema_access_template(context)}",
      binding
    )
  ...
  def ensure_test_file_exists(%Context{test_file: test_file} = context, paths, binding) do
    unless Context.pre_existing_tests?(context) do
      Mix.Generator.create_file(
        test_file,
        Mix.Maxo.eval_from(paths, "priv/templates/maxo.gen.context/context_test.exs", binding)
      )
    end
  end
  ...
  defp inject_tests(%Context{test_file: test_file} = context, paths, binding) do
    ...
    paths
    |> Mix.Maxo.eval_from("priv/templates/maxo.gen.context/test_cases.exs", binding)
  ...
  def ensure_test_fixtures_file_exists(
    ...
    unless Context.pre_existing_test_fixtures?(context) do
      Mix.Generator.create_file(
        ...
        Mix.Maxo.eval_from(
          paths,
          "priv/templates/maxo.gen.context/fixtures_module.ex",
          binding
        )
      )
    end
  ...
  defp inject_test_fixture(
    ...
    paths
    |> Mix.Maxo.eval_from("priv/templates/maxo.gen.context/fixtures.ex", binding)
    |> Mix.Maxo.prepend_newline()
  ...
  def raise_with_help(msg) do
    ...
    mix maxo.gen.html, maxo.gen.json, maxo.gen.live, and maxo.gen.context
    ...
    For example:
        ...
        mix maxo.gen.html Accounts User users name:string
        mix maxo.gen.json Accounts User users name:string
        mix maxo.gen.live Accounts User users name:string
        mix maxo.gen.context Accounts User users name:string
