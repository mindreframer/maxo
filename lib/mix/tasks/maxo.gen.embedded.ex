defmodule Mix.Tasks.Maxo.Gen.Embedded do
  ...
  Generates an embedded Ecto schema for casting/validating data outside the DB.
      ...
      mix maxo.gen.embedded Blog.Post title:string views:integer
  ...
  the type makes it default to `:string`:
      ...
      mix maxo.gen.embedded Blog.Post title views:integer

  The following types are supported:

  #{for attr <- Mix.Maxo.Schema.valid_types(), do: "  * `#{inspect(attr)}`\n"}
    * `:datetime` - An alias for `:naive_datetime`
  """
  use Mix.Task

  alias Mix.Maxo.Schema
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise(
        "mix maxo.gen.embedded must be invoked from within your *_web application root directory"
      )
    end
  ...
  def raise_with_help(msg) do
    ...
    mix maxo.gen.embedded expects a module name followed by
    any number of attributes:

        mix maxo.gen.embedded Blog.Post title:string
    """)
  end
  ...
  def copy_new_files(%Schema{} = schema, paths, binding) do
    ...
    Mix.Maxo.copy_from(paths, "priv/templates/maxo.gen.embedded", binding, files)
