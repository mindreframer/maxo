defmodule Mix.Tasks.Maxo.Gen.Socket do
  ...
  You can then generated channels with `mix maxo.gen.channel`.
  """
  use Mix.Task
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise(
        "mix maxo.gen.socket must be invoked from within your *_web application root directory"
      )
    end
    ...
    binding = Mix.Maxo.inflect(socket_name)

    existing_channel =
      if pre_existing_channel do
        channel_binding = Mix.Maxo.inflect(pre_existing_channel)
    ...
    Mix.Maxo.check_module_name_availability!(binding[:module] <> "Socket")

    Mix.Maxo.copy_from(paths(), "priv/templates/maxo.gen.socket", binding, [
    ...
    Add the socket handler to your `#{Mix.Maxo.web_path(context_app, "endpoint.ex")}`, for example:
  ...
  defp raise_with_help do
    Mix.raise("""
    mix maxo.gen.socket expects the module name:
