defmodule Mix.Tasks.Maxo.Gen.Auth do
  ...
  module name, for example:
      ...
      $ mix maxo.gen.auth Accounts User users --web Warehouse
  ...
  workflows:
      ...
      $ mix maxo.gen.auth Store User users
      $ mix maxo.gen.auth Backoffice Admin admins
  ...
  a `--table` option may be provided. For example:
      ...
      $ mix maxo.gen.auth Accounts User users --table accounts_users
  ...
  alias Mix.Maxo.{Context, Schema}
  alias Mix.Tasks.Maxo.Gen
  alias Mix.Tasks.Maxo.Gen.Auth.{HashingLibrary, Injector, Migration}
  ...
  def run(args, test_opts \\ []) do
    if Mix.Project.umbrella?() do
      Mix.raise("mix maxo.gen.auth can only be run inside an application directory")
    end
  ...
  defp validate_required_dependencies! do
    unless Code.ensure_loaded?(Ecto.Adapters.SQL) do
      raise_with_help("mix maxo.gen.auth requires ecto_sql", :phx_generator_args)
    end

    if generated_with_no_html?() do
      raise_with_help("mix maxo.gen.auth requires phoenix_html", :phx_generator_args)
    end
  end
  ...
  defp prompt_for_conflicts(context) do
    ...
    |> Mix.Maxo.prompt_for_conflicts()
  end

  defp files_to_be_generated(%Context{schema: schema, context_app: context_app} = context) do
    singular = schema.singular
    web_pre = Mix.Maxo.web_path(context_app)
    # web_test_pre = Mix.Maxo.web_test_path(context_app)
    migrations_pre = Mix.Maxo.context_app_path(context_app, "priv/repo/migrations")
    web_path = to_string(schema.web_path)
    controller_pre = Path.join([web_pre, "controllers", web_path])
    web_test_pre = controller_pre
  ...
  defp copy_new_files(%Context{} = context, binding, paths) do
    ...
    Mix.Maxo.copy_from(paths, "priv/templates/maxo.gen.auth", binding, files)
  ...
  defp inject_context_functions(%Context{file: file} = context, paths, binding) do
    ...
    paths
    |> Mix.Maxo.eval_from("priv/templates/maxo.gen.auth/context_functions.ex", binding)
    |> prepend_newline()
  ...
  defp inject_tests(%Context{test_file: test_file} = context, paths, binding) do
    ...
    paths
    |> Mix.Maxo.eval_from("priv/templates/maxo.gen.auth/test_cases.exs", binding)
    |> prepend_newline()
  ...
  defp inject_context_test_fixtures(
    ...
    paths
    |> Mix.Maxo.eval_from("priv/templates/maxo.gen.auth/context_fixtures_functions.ex", binding)
    |> prepend_newline()
  ...
  defp inject_conn_case_helpers(%Context{} = context, paths, binding) do
    ...
    paths
    |> Mix.Maxo.eval_from("priv/templates/maxo.gen.auth/conn_case.exs", binding)
    |> inject_before_final_end(test_file)
  ...
  defp inject_routes(%Context{context_app: ctx_app} = context, paths, binding) do
    ...
    paths
    |> Mix.Maxo.eval_from("priv/templates/maxo.gen.auth/routes.ex", binding)
  ...
  defp maybe_inject_mix_dependency(%Context{context_app: ctx_app} = context, %HashingLibrary{
    ...
    file_path = Mix.Maxo.context_app_path(ctx_app, "mix.exs")
  ...
  defp inject_config(context, %HashingLibrary{} = hashing_library) do
    file_path =
      if Mix.Maxo.in_umbrella?(File.cwd!()) do
        Path.expand("../../")
      else
        File.cwd!()
      end
  ...
  defp raise_with_help(msg, :general) do
    ...
    mix maxo.gen.auth expects a context module name, followed by
  ...
  defp raise_with_help(msg, :phx_generator_args) do
    ...
    mix maxo.gen.auth must be installed into a Phoenix 1.5 app that
  ...
  defp raise_with_help(msg, :hashing_lib) do
    ...
    mix maxo.gen.auth supports the following values for --hashing-lib
