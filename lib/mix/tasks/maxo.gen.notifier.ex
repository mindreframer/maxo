defmodule Mix.Tasks.Maxo.Gen.Notifier do
  @shortdoc "Generates a notifier that delivers emails by default"

  @moduledoc """
  Generates a notifier that delivers emails by default.

      $ mix maxo.gen.notifier Accounts User welcome_user reset_password confirmation_instructions
  ...
  generated in a different app under an umbrella.
      ...
      $ mix maxo.gen.notifier Accounts User welcome_user --context-app marketing
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise(
        "mix maxo.gen.notifier must be invoked from within your *_web application root directory"
      )
    end

    {context, notifier_module, messages} = build(args)

    inflections = Mix.Maxo.inflect(notifier_module)
  ...
  def raise_with_help(msg) do
    ...
    mix maxo.gen.notifier expects a context module name, followed by a
    ...
    For example:
        ...
        mix maxo.gen.notifier Accounts User welcome reset_password
  ...
  defp copy_new_files(%Context{} = context, binding, paths) do
    ...
    Mix.Maxo.copy_from(paths, "priv/templates/maxo.gen.notifier", binding, files)
