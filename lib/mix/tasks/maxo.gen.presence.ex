defmodule Mix.Tasks.Maxo.Gen.Presence do
  @shortdoc "Generates a Presence tracker"

  @moduledoc """
  Generates a Presence tracker.

      $ mix maxo.gen.presence
      $ mix maxo.gen.presence MyPresence
  ...
  def run([alias_name]) do
    if Mix.Project.umbrella?() do
      Mix.raise(
        "mix maxo.gen.presence must be invoked from within your *_web application root directory"
      )
    end
    ...
    inflections = Mix.Maxo.inflect(alias_name)
    ...
    Mix.Maxo.copy_from(paths(), "priv/templates/maxo.gen.presence", binding, files)
