defmodule Mix.Maxo.Context do
  ...
  def new(context_name, %Schema{} = schema, opts) do
    ...
    base = Module.concat([Mix.Maxo.context_base(ctx_app)])
    module = Module.concat(base, context_name)
    alias = Module.concat([module |> Module.split() |> List.last()])
    basedir = Maxo.Naming.underscore(context_name)
    basename = Path.basename(basedir)
    dir = Mix.Maxo.context_lib_path(ctx_app, basedir)
    file = dir <> "/" <> basename <> ".ex"
    test_dir = Mix.Maxo.context_test_path(ctx_app, basedir) <> "/" <> basename
    test_file = test_dir <> "_test.exs"
    test_fixtures_dir = Mix.Maxo.context_app_path(ctx_app, "test/support/fixtures")
  ...
  defp web_module do
    ...
    cond do
      Mix.Maxo.context_app() != Mix.Maxo.otp_app() ->
        Module.concat([base])
