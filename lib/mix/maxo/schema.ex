defmodule Mix.Maxo.Schema do
  ...
  def new(schema_name, schema_plural, cli_attrs, opts) do
    ...
    types = types(attrs)
    web_namespace = opts[:web] && Maxo.Naming.camelize(opts[:web])
    web_path = web_namespace && Maxo.Naming.underscore(web_namespace)
    ...
    %Schema{
      ...
      human_singular: Maxo.Naming.humanize(singular),
      human_plural: Maxo.Naming.humanize(schema_plural),
  ...
  * In case it is a boolean, we keep it as false
  ...
  For example:
      ...
      mix maxo.gen.schema Comment comments body:text status:enum:published:unpublished
  """
  ...
  defp validate_attr!({name, :array}) do
    ...
    For example:
        ...
        mix maxo.gen.schema Post posts settings:array:string
    """)
  end
  ...
  defp partition_attrs_and_assocs(schema_module, attrs) do
      ...
      Enum.split_with(attrs, fn
        ...
        {key, :references} ->
          ...
          For example:
              ...
              mix maxo.gen.schema Comment comments body:text post_id:references:posts
          """)
    ...
    assocs =
      Enum.map(assocs, fn {key_id, {:references, source}} ->
        ...
        module = Module.concat(base ++ [Maxo.Naming.camelize(key)])
