defmodule <%= inspect context.web_module %>.<%= inspect Module.concat(schema.web_namespace, schema.alias) %>ControllerTest do
  ...
  @create_attrs <%= Mix.Maxo.to_text schema.params.create %>
  @update_attrs <%= Mix.Maxo.to_text schema.params.update %>
  @invalid_attrs <%= Mix.Maxo.to_text (for {key, _} <- schema.params.create, into: %{}, do: {key, nil}) %>
  ...
  describe "update <%= schema.singular %>" do
    ...
    test "redirects when data is valid", %{conn: conn, <%= schema.singular %>: <%= schema.singular %>} do
      ...
      assert html_response(conn, 200) =~ <%= inspect Mix.Maxo.Schema.default_param(schema, :update) %><% else %>
