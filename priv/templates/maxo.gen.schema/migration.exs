defmodule <%= inspect schema.repo %>.Migrations.Create<%= Macro.camelize(schema.table) %> do
  use Appbox.Migration, <%= inspect(NaiveDateTime.local_now()) %>
...
<% end %><%= for {k, v} <- schema.attrs do %>      add <%= inspect k %>, <%= inspect Mix.Maxo.Schema.type_for_migration(v) %><%= schema.migration_defaults[k] %>
