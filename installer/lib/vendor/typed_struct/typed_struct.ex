defmodule MaxoNew.TypedStruct do
  @accumulating_attrs [
    :ts_plugins,
    :ts_plugin_fields,
    :ts_fields,
  ...
  defmacro __using__(_) do
    quote do
      alias MaxoNew.TypedStruct
      import MaxoNew.TypedStruct, only: [typedstruct: 1, typedstruct: 2]
    end
  end
  ...
  defmacro typedstruct(opts \\ [], do: block) do
    ast = MaxoNew.TypedStruct.__typedstruct__(block, opts)
  ...
  def __typedstruct__(block, opts) do
    quote do
      ...
      import MaxoNew.TypedStruct
      unquote(block)

      @enforce_keys @ts_enforce_keys
      defstruct @ts_fields

      MaxoNew.TypedStruct.__type__(@ts_types, unquote(opts))
    end
  end
  ...
  defmacro field(name, type, opts \\ []) do
    quote bind_quoted: [name: name, type: Macro.escape(type), opts: opts] do
      MaxoNew.TypedStruct.__field__(name, type, opts, __ENV__)
    end
  end
