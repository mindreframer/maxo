defmodule MaxoNew.Virtfs do
  use GenServer
  alias MaxoNew.Virtfs.Backend
  alias MaxoNew.Virtfs.Dumper
  alias MaxoNew.Virtfs.FS
  alias MaxoNew.Virtfs.Loader
  ...
  def handle_call({:dump, path}, _from, %FS{} = fs) do
    res = Dumper.run(fs, path)
    {:reply, res, fs}
  end

  @impl true
  def handle_call({:load, path}, _from, %FS{} = fs) do
    with {:ok, res} <- Loader.run(fs, path) do
      {:reply, :ok, res}
    end
  end
