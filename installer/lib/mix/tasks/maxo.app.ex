defmodule Mix.Tasks.Maxo.App do
  @moduledoc """
  Creates a new Phoenix project.

  It expects the path of the project as an argument.

      $ mix maxo.app PATH [--module MODULE] [--app APP]
  ...
  `maxo.gen.html`, `maxo.gen.json`, `maxo.gen.live`, and `maxo.gen.context`
  ...
  `maxo.gen.html` will no longer work, as important HTML components
  will be missing.

  ## Installation

  `mix maxo.app` by default prompts you to fetch and install your
  dependencies. You can enable this behaviour by passing the
  `--install` flag or disable it with the `--no-install` flag.

  ## Examples

      $ mix maxo.app hello_world

  Is equivalent to:

      $ mix maxo.app hello_world --module HelloWorld

  Or without the HTML and JS bits (useful for APIs):

      $ mix maxo.app ~/Workspace/hello_world --no-html --no-assets

  ```
  """
  use Mix.Task
  alias MaxoNew.{Generator, Project, Single}
  ...
  def run(argv) do
    ...
    case OptionParser.parse!(argv, strict: @switches) do
      {_opts, []} -> Mix.Tasks.Help.run(["maxo.app"])
      {opts, [base_path | _]} -> generate(base_path, Single, :project_path, opts)
    end
  end
  ...
  def unit_test_args(command) do
    ...
    case OptionParser.parse!(argv, strict: @switches) do
      {_opts, []} -> Mix.Tasks.Help.run(["maxo.app"])
      {opts, [base_path | _]} -> do_generation(base_path, Single, :project_path, opts)
    end
  end
  ...
  def unit_test_project(command) do
    ...
    case OptionParser.parse!(argv, strict: @switches) do
      {_opts, []} -> Mix.Tasks.Help.run(["maxo.app"])
      {opts, [base_path | _]} -> do_full_project_struct(base_path, Single, :project_path, opts)
    end
  end
